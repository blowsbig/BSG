...
*Setup instructions for a gitian build of MRQ using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the MRQ
...
Clone the git repositories for BSG and gitian and then checkout the BSG version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/BSG-crypto/BSG.git
cd BSG
...
Follow the instructions in [doc/release-process.md](release-process.md) in the BSG repository
...
To build MRQ (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the BSG repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/BSG/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/BSG-crypto/BSG
...
For example:
```bash
URL=https://github.com/blowsbig/BSG.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit BSG=${COMMIT} --url BSG=${URL} ../BSG/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit BSG=${COMMIT} --url BSG=${URL} ../BSG/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit BSG=${COMMIT} --url BSG=${URL} ../BSG/contrib/gitian-descriptors/gitian-osx.yml
```
...
and follow the steps in the build process as normal.
    ...
    gpg: skipped "BSGcoin": secret key not available

When you execute `gsign` you will get an error from GPG, which can be ignored. Copy the resulting `.assert` files
in `gitian.sigs` to your signing machine and do

```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/BSG-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/BSG-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/BSG-build.assert
```
...
[BSG/gitian.sigs](https://github.com/BSG-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
