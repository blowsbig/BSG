...
TMP_FOLDER=$(mktemp -d)
CONFIG_FILE='BSG.conf'
CONFIGFOLDER='/root/.BSG'
COIN_DAEMON='BSGd'
COIN_CLI='BSG-cli'
COIN_PATH='/usr/local/bin/'
COIN_REPO='https://github.com/blowsbig/BSG.git'
COIN_TGZ='https://github.com/blowsbig/BSG/releases/download/v3.0.0.0/mirq-3.0.0-x86_64-linux-signed.tar.gz'
...
function get_ip() {
  declare -a NODE_BSG
  for BSG in $(netstat -i | awk '!/Kernel|Iface|lo/ {print $1," "}')
  do
    NODE_BSG+=($(curl --interface $BSG --connect-timeout 2 -s4 icanhazip.com))
  done
